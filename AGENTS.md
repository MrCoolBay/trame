...
| Bibliotheque de composants | `gpui-component` **0.5.1**, crates.io | Le champ multi-ligne valait la dependance ; le reste non. `Styled` expose et `.refine_style()` qui raffine par-dessus le preset donnent l'echappatoire — on habille la bibliotheque au lieu d'etre habille par elle. **Champ multi-ligne : `auto_grow(min, max)` en un appel, ou `multi_line(true).rows(n)` **plus une hauteur sur l'element** en trois.** | [0028](docs/adr/0028-adoption-de-gpui-component.md) |
