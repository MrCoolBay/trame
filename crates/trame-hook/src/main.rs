...
//!   le reason.
//! - **1 avec un message sur `stderr`** — Trame n'a pas pu check. **Deny aussi**, mais
//!   bruyant : c'est une panne, pas une politique.
//!
//! Le troisieme cas est la raison d'etre de ce file. Un hook qui sortirait 0 sans avoir
...
fn main() -> ExitCode {
    ...
    if let Err(error) = std::io::stdin().read_to_string(&mut payload) {
        return deny(&format!("payload illisible sur stdin : {error}"));
    }

    // Le projet est passe par la line de commande : c'est Trame qui ecrit le file de
    // reglages, donc Trame qui sait de quel projet il s'agit.
    let Some(projet) = std::env::args().nth(1) else {
        return deny("usage : trame-hook <projet> — appele sans projet");
    };

    let socket = match trame_hook::socket_path(&projet) {
        Ok(path) => path,
        Err(error) => return deny(&error.reason()),
    };

    match trame_hook::ask(&socket, &payload) {
        Ok(decision) => {
            if let Some(json) = decision.to_json() {
                // Un echec d'ecriture sur stdout laisserait la CLI conclure « pas d'objection ».
                if let Err(error) = std::io::stdout()
                    .write_all(json.as_bytes())
                    .and_then(|()| std::io::stdout().flush())
                {
                    return deny(&format!("refus non transmis a la CLI : {error}"));
                }
            }
            ExitCode::SUCCESS
        }
        // ★ Toute erreur mene ici, et ici on refuse.
        Err(error) => deny(&error.reason()),
    }
}
...
fn deny(reason: &str) -> ExitCode {
    let _ = writeln!(std::io::stderr(), "trame-hook : {reason}");
    ExitCode::FAILURE
}
